module Jekyll
  class EntryFilter
    ...
    def derive_base_directory(site, base_dir)
      base_dir[site.source] = "" if base_dir.start_with?(
        site.source
      )
    ...
    # --
    def symlink?(entry)
      site.safe && File.symlink?(entry) && symlink_outside_site_source?(
        entry
      )
    end
    ...
    def glob_include?(enum, e)
      ...
      enum.any? do |exp|
        ...
        # User who sends a Regexp knows what they want to exclude, so let
        # them send a Regexp to exclude files, we will not bother caring if
        # it works or not, it's on them at this point.
        ...
        if exp.is_a?(Regexp)
          ...
          # If it's a directory they want to exclude, AKA ends with a "/"
          # then we will go on to check and see if the entry falls within that
          # path and exclude it if that's the case.
          ...
          if e.end_with?("/")
            ...
            File.fnmatch?(item, entry) || entry.to_path.start_with?(
              item
            )
          end
        end
